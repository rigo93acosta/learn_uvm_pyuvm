...
async def test_multiple_clock_domains(dut):
    ...
    clk_fast = Clock(dut.clk, 10, units="ns")
    ...
    for i in range(3):
        await Timer(period_slow, units="ns")
        cocotb.log.info(
            f"Pulso del dominio lento #{i + 1} a los {cocotb.utils.get_sim_time(units='ns')}ns"
        )
...
async def test_clock_synchronization(dut):
    ...
    for i in range(1, 4):
        ...
        await RisingEdge(
            dut.clk
        )  # Sincronización con el flanco de subida para capturar el dato
        ...
        cocotb.log.info(
            f"Dato 0x{dut.d.value.to_unsigned():02X} sincronizado y capturado correctamente."
        )
