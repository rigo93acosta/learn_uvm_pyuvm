...
async def test_register_reset(dut):
    ...
    clock = Clock(dut.clk, 10, unitss="ns")
    cocotb.start_soon(clock.start())

    await reset_dut(dut)

    # Verify reset state
    assert int(dut.q.value) == 0, "Register should be reset to 0"
...
async def test_register_write(dut):
    ...
    assert int(dut.q.value) == 0xAB, "Register should store written value"
...
async def test_register_enable(dut):
    ...
    await RisingEdge(dut.clk)
    await Timer(1, unitss="ns")
    assert int(dut.q.value) == 0x12, "Register should not update when enable is off"
...
async def test_register_all_values(dut):
    ...
    for value in test_values:
        ...
        assert int(dut.q.value) == value, (
            f"Failed for value 0x{value:02X}, got 0x{int(dut.q.value):02X}"
        )
